...
class CreateInternalPassportByStaffAPITests(APITestCase):
    def setUp(self):
        self.path =  "/api/staff/create-internal-passport/"
        ...
        self.wrong_task = TaskFactory(user=self.user,
    ...
    def test_create_internal_passport_user_already_has_passport(self):
        self.user.passport = PassportFactory()
        self.user.save()
        response = self.client.post(
            path=f"{self.path}{self.task.pk}/",
            data=self.valid_data,
    ...
    def test_create_internal_passport_task_already_processed(self):
        self.task.status = 1
        self.task.save()
        response = self.client.post(
            path=f"{self.path}{self.task.pk}/",
    ...
    def test_create_internal_passport_wrong_task(self):
        response = self.client.post(
            path=f"{self.path}{self.wrong_task.pk}/",
            data=self.valid_data,
            format='json'
        )
    ...
    def test_create_internal_passport_address_incorrect(self):
        ...
        response = self.client.post(
            path=f"{self.path}{incorrect_task.pk}/",
            data=self.valid_data,
            format='json'
        )
        self.assertEqual(response.status_code, status.HTTP_404_NOT_FOUND)
    ...
    def test_create_internal_passport_without_authority(self):
        response = self.client.post(
            path=f"{self.path}{self.task.pk}/",
            data={
                "date_of_issue": self.valid_data['date_of_issue'],
    ...
    def test_create_internal_passport_authority_incorrect(self):
        response = self.client.post(
            path=f"{self.path}{self.task.pk}/",
            data={
                "authority": 10,
    ...
    def test_create_internal_passport_date_format_incorrect(self):
        response = self.client.post(
            path=f"{self.path}{self.task.pk}/",
            data={
                "authority": self.valid_data['authority'],
    ...
    def test_create_internal_passport_date_of_issue_incorrect(self):
        response = self.client.post(
            path=f"{self.path}{self.task.pk}/",
            data={
                "authority": self.valid_data['authority'],
    ...
    def test_create_internal_passport_date_of_expiry_incorrect(self):
        response = self.client.post(
            path=f"{self.path}{self.task.pk}/",
            data={
                "authority": self.valid_data['authority'],
    ...
    def test_create_internal_passport_no_data(self):
    ...
    def test_create_internal_passport_task_not_found(self):
    ...
    def test_create_internal_passport_user_no_access(self):
